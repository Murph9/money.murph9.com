...
const entryList = computed(() => {
  ...
  const prev = Context.value.calc.reportFor(
    props.type,
    DayTypeLib.offsetDateBy(props.date, props.type, -1)
  );
...
<template>
  <h4 class="float-start">
    Report: {{ DayType[props.type] }} of {{ props.date.toLocaleDateString() }}
  </h4>
  ...
  <div class="float-end">
    <div class="input-group">
      <button
        :class="showIncome ? 'btn-primary' : 'btn-outline-primary'"
        class="btn btn-sm"
      ...
      <button
        :class="showDiff ? 'btn-primary' : 'btn-outline-primary'"
        class="btn btn-sm"
  ...
  <div class="input-group">
    <button
      :class="activeTab == TAB_GRAPH ? 'btn-secondary' : 'btn-outline-secondary'"
      class="btn"
      @click="activeTab = TAB_GRAPH"
    >
      GraphView
    </button>
    <button
      :class="activeTab == TAB_TABLE ? 'btn-secondary' : 'btn-outline-secondary'"
      class="btn"
      @click="activeTab = TAB_TABLE"
    >
      GroupedTable
    </button>
    <button
      :class="activeTab == TAB_RAW ? 'btn-secondary' : 'btn-outline-secondary'"
      class="btn"
      @click="activeTab = TAB_RAW"
    >
      RawTable
    </button>
  </div>
  ...
  <ReportGraph
    v-if="activeTab == TAB_GRAPH && showIncome"
    :data="incomeList"
    :max-count="incomeList.length"
    :show-diff="showDiff"
    style="max-height: 100px"
  ></ReportGraph>
  <ReportGraph
    v-if="activeTab == TAB_GRAPH"
    :data="expenseList"
    :max-count="maxCount"
    :show-diff="showDiff"
  ></ReportGraph>
  <ReportTable
    v-if="activeTab == TAB_TABLE && showIncome"
    :data="incomeList"
    :max-count="incomeList.length"
    :show-diff="showDiff"
  ></ReportTable>
  <ReportTable
    v-if="activeTab == TAB_TABLE"
    :data="expenseList"
    :max-count="maxCount"
    :show-diff="showDiff"
  ></ReportTable>
